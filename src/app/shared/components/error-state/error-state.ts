...
@Component({
  ...
  template: `
    <div class="flex flex-col items-center justify-center gap-3 rounded-xl border border-red-900/40 bg-red-950/20 px-6 py-16 text-center">
      ...
      <p class="text-base font-medium text-brand-deep-100">{{ error().message }}</p>
      @if (showRetry()) {
        <button
          type="button"
          (click)="retry.emit()"
          class="mt-1 rounded-full bg-brand-primary px-4 py-1.5 text-sm font-medium text-brand-cream transition hover:brightness-110"
        >
          Try again
        </button>
      }
